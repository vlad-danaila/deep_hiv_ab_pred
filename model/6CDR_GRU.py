...
class CDR_GRU(t.nn.Module):

    def __init__(self, conf, cdr_module, embeddings_matrix = None):
        super().__init__()
        ...
        self.cdr = t.nn.ModuleList([ cdr_module(self.cdr_in_lens[i], RNN_HIDDEN_SIZE) for i in range(len(CDR_LENGHTS)) ])
    ...
    def forward_antibodyes(self, ab_cdr, ab_cdr_mask, ab_cdr_pos):
        ...
        for i in range(len(CDR_LENGHTS)):
            ...
            cdr_out_list.append(self.cdr[i](tensor_in))
            begin = end
