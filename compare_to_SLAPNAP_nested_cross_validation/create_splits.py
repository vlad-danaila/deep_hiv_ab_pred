...
def create_splits_to_compare_with_slapnap(catnap, fold):
    ...
    for antibody, (virus_ids, folds, nested_folds) in slapnap_data.items():
        ...
        train_viruses = np.array([ v for v in virus_ids if v not in test_viruses ])
        train_data = map_to_ids(antibody, train_viruses, catnap)
        test_splits = { 'train': train_data, 'test': test_data }

        cv_splits = cross_validation_splits(train_viruses, nested_folds[fold], antibody, catnap)
