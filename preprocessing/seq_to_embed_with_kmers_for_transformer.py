...
from deep_hiv_ab_pred.preprocessing.sequences_to_embedding import read_virus_fasta_sequences, read_virus_pngs_mask, sequence_to_indexes
...
def parse_ab_sites(kmer_len, kmer_stride):
...
def parse_catnap_sequences_to_embeddings(kmer_len, kmer_stride):
    virus_seq = read_virus_fasta_sequences(VIRUS_FILE, kmer_len, kmer_stride)
    virus_pngs_mask = read_virus_pngs_mask(VIRUS_WITH_PNGS_FILE, kmer_len, kmer_stride)
    abs = parse_ab_sites(kmer_len, kmer_stride)
    ...
    for ab_id, seq in abs.items():
        ...
        seq = np.pad(seq, ((0, pad_amnt), (0, 0)), 'constant', constant_values = amino_to_index['X'])
        abs[ab_id] = seq

    for v in virus_seq:
        pad_amnt = virus_max_len - len(virus_seq[v])
        seq = np.pad(virus_seq[v], ((0, pad_amnt), (0, 0)), 'constant', constant_values = amino_to_index['X'])
...
if __name__ == '__main__':
    virus_seq, abs, virus_max_len, ab_max_len = parse_catnap_sequences_to_embeddings(50, 25)
